...
class FRCNN(object):
    ...
    #---------------------------------------------------#
    def generate(self):
        model_path = os.path.expanduser(self.model_path)  # 可以将特殊的表示主目录的符号扩展为实际的路径字符串
        ...
        # 否则先构建模型再载入
        self.model_rpn,self.model_classifier = frcnn.get_frcnn_predict_model(self.config,self.num_classes)
    ...
    def detect_image(self, image):
        ...
        width,height = get_new_img_size(old_width, old_height)  # 将短边缩放为600，并计算比列系数缩放长边得到长边得尺寸
        ...
        # preds[0].shape=(1,12996,1)
        # preds[1].shape=(1,12996,4) （△x,△y,△w,△h)
        # preds[2].shape=(1,38,38,1024)
       
        preds = self.model_rpn.predict(photo)
      
        # 将预测结果进行解码，在原图中生成归一化处理后的先验框(left,top,right,bottom)
        anchors = get_anchors(self.get_img_output_length(width,height),width,height)
        # rpn_results[0].shape=(300,6)
        rpn_results = self.bbox_util.detection_out(preds,anchors,1,confidence_threshold=0)
        # R.shape=(300,4) (left,top,right,bottom)
        R = rpn_results[0][:, 2:]
        # 转换为featuremap上的坐标
        ...
        for jk in range(R.shape[0]//self.config.num_rois + 1):
            ROIs = np.expand_dims(R[self.config.num_rois*jk:self.config.num_rois*(jk+1), :], axis=0) #shape=(1,32,4)
